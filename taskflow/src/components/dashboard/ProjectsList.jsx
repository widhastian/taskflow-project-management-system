    ...
    <div className="bg-white rounded-2xl border border-slate-100 shadow-sm flex flex-col">
      ...
      <div className="flex-1 divide-y divide-slate-100">
        {recentProjects.map((project) => (
          <div key={project.id} className="p-3 hover:bg-slate-50 transition-colors">
            <div className="flex items-center justify-between mb-1.5">
              <h4 className="text-sm font-medium text-slate-800">{project.name}</h4>
              <span className={`px-1.5 py-0.5 text-[9px] font-medium rounded-full ${getStatusColor(project.status)}`}>
                {project.status}
              </span>
            </div>
            <p className="text-[11px] text-slate-500 mb-2">{project.description}</p>
