...
function TodayTasks() {
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 flex-1 flex flex-col min-h-0">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h2 className="text-sm font-semibold text-slate-800">Today's Task</h2>
        ...
        {tasks.map((task) => (
          <div key={task.id} className={`flex items-start gap-2 p-2 rounded-lg transition-colors ${task.completed ? 'bg-green-50/50' : 'hover:bg-slate-50'}`}>
            <button 
              ...
              className={`mt-0.5 w-4 h-4 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition-all ${task.completed ? 'bg-green-500 border-green-500' : 'border-slate-300 hover:border-primary-500'}`}
            >
              {task.completed && <CheckCircle2 className="w-2.5 h-2.5 text-white" />}
            </button>
            <div className="flex-1 min-w-0">
              <p className={`text-[11px] font-medium ${task.completed ? 'line-through text-slate-400' : 'text-slate-800'}`}>{task.title}</p>
              <div className="flex items-center gap-1 mt-0.5">
                <span className="text-[9px] text-slate-500">{task.project}</span>
